    ...
    public class Common
    {
            ...
            string toMail)
        {
            ...
            try
            {
                ...
                message.To.Add(toMail); // Địa chỉ email nhận
                message.Subject = subject; // Tiêu đề email
                message.IsBodyHtml = true; // Email hỗ trợ HTML
                message.Body = content; // Nội dung email
                smtp.Send(message); // Gửi email qua máy chủ SMTP của Gmail
                rs = true;
            }
            catch (Exception ex)
            {
